...
def plot_controls(result_path):
    ...
    for file in os.listdir(result_path):
        if file.endswith('best_controls.json'):
            file_path = os.path.join(result_path, file)

    save_name = "Controls_" + opt_name

    controls = readjson(file_path)

    time_grid = []
    pulse = []

    for data_name in controls.keys():
...
def test_dCRAB_Fourier_NM_OneQubit():
    ...
    optimization_dictionary = readjson(os.path.join(folder, "dCRAB_Fourier_NM_OneQubit_json_dump.json"))
