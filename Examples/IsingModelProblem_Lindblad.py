...
class IsingModel(AbstractFoM):
    ...
    def _solve_LME(self, H, time_grid):
        ...
        def LME(t, rho, args):
            dRho = -1j * (H(t) @ rho - rho @ H(t).conj()) \
                   + self.Lindbladian @ rho @ self.Lindbladian.conj() \
                   - 1/2 * self.Lindbladian @ self.Lindbladian.conj() @ rho \
                   - 1/2 * rho @ self.Lindbladian @ self.Lindbladian.conj()
            return dRho

        term = ODETerm(LME)
        solver = Dopri5()
        dt = time_grid[-1] / len(time_grid) / 2
        solution = diffeqsolve(term, solver, t0=time_grid[0], t1=time_grid[-1], dt0=dt, y0=self.rho_0)
        print(self.rho_0)
        print(solution.ys[-1])
