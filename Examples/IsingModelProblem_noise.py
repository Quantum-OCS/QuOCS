...
class IsingModel(AbstractFoM):
    ...
    def get_FoM(self,
        ...
        noise = 0.01
        rho_final = U_final @ self.rho_0 @ U_final.T.conj() + noise*np.random.rand(len(self.rho_0),len(self.rho_0))
        #rho_final = U_final @ self.rho_0 @ U_final.T.conj()
        rho_final *= 1/np.trace(rho_final)        # Calculate the fidelity
        fidelity = fom_funct(rho_final, self.rho_target)
        self.FoM_list.append(fidelity)
        return {"FoM": -fidelity, "std": 5*noise}
