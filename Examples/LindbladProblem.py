...
class TLSProblem(AbstractFoM):
    ...
    See the example 6 (equations (17), (96 - 89)) in https://doi.org/10.1063/1.5115323
    """
...
def fidelity_funct(rho_evolved, rho_aim):
    ...
    :return: fidelity
    """
    return jnp.abs(jnp.trace(jnp.sqrtm(jnp.sqrtm(rho_evolved) @ rho_aim @ jnp.sqrtm(rho_evolved)))) ** 2
...
def pw_evolution_final(rho_0, L_const, L_drive, drive, time_grid):
    ...
    def body_fun(i, val):
        ...
        U_curr = jsp.linalg.expm(-1.0j * L_curr * dt)
        return U_curr @ val
...
# muh = TLSProblem()
# muh.get_FoM(pulses=[np.array([0.1]*20)], parameters=[], timegrids=[np.linspace(0, 1, 20)])
