...
class OnePlusOne(DirectSearchMethod):
    callback: callable
    ...
    def run_dsm(self,
        ...
        optimiser = ng.optimizers.OnePlusOne(parametrization=parametrization, budget=None)

        # Specify the batch size
        batch_size = len(x0)+1 if len(x0) < 7 else 7 # according to nevergrad documentation it should be <8
        ...
        while not self.sc_obj.is_converged:
            ...
            logger.info("Nevergrad 1+1 - Average FoM: {} / Std Dev: {}".format(np.mean(fsim), np.std(fsim)))
